...
export default function Cell({
    ...
    const positionClick = () => {
        if (selectedPosition && selectedPosition.length > 0) {
            document
                .getElementById(selectedPosition[0].toString() + selectedPosition[1].toString())!
                .classList.toggle("bg-sky-800")
        }
        document.getElementById(row.toString() + col.toString())!.classList.toggle("bg-sky-800")
    ...
    return (
        <div id={row.toString() + col.toString()} className={borderClassStyle()} onClick={positionClick}>
            {renderCell()}
        </div>
    )
}
